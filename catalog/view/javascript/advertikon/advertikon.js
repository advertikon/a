( function outerClosure( $ ) {
	 ...
	 * @returns {void}
	 */
	ADK.initSwitchable = function is( element ) {
	 ...
	 * @returns {void}
	 */
	ADK.initIris = function is( element ) {
		$( element ).iris( {
			target: $( element ).parent()
				.parent(),
			width:  300,
			hide:   true,
			change: function colorPicherChange( evt, ui ) {
	...
	ADK.Notification = function n() {
		...
		if ( !document.getElementById( "adk-notification-container" ) ) {
			container = $(
	...
	ADK.progress = function( url, data ) {
		var
			deff = new $.Deferred(),
			ret = null,
			doneCb = null,
