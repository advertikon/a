...
class Advertikon {
	...
	public function __construct() {
		...
		$this->is_console = $this->console->active;
	}

	public static function instance( $code = null ) {
		if ( 'Advertikon\Advertikon' === $code ) {
			$code = 'main';
		}
	...
	public function install() {
		$q = $this->db->query( "SELECT * FROM " . DB_PREFIX . "url_alias WHERE `keyword` = 'adk_log'" );

		if ( $q && $q->num_rows < 1 ) {
			$route = $this->type . '/' . $this->code . '/log';

			$this->db->query( "INSERT INTO " . DB_PREFIX . "url_alias SET `keyword` = 'adk_log', `query` = '" . $route . "'" );
		}
	}

	public function uninstall() {

		// TODO: will delete alias for the second installation as well
		$this->db->query( "DELETE FROM " . DB_PREFIX . "url_alias WHERE `keyword` = 'adk_log'" );
	}
	...
	public function option() {
		...
		if ( ! $this->option ) {
			...
			$this->option = new Option( $namespace );
		}
	...
	public function get_order_products( $order_id ) {
		...
		if( ! $this->has_in_cache( 'order_products/' . $order_id ) ) {
			$query = $this->db->query(
				"SELECT *, op.quantity as quantity FROM `" . DB_PREFIX . "order_product` `op`
	 ...
	 */
	public function log() {
		if ( ! $this->is_console && self::LOGS_DISABLE === $this->log_strictness ) {
			return;
		}
		...
		if ( ! $this->is_console && $this->log_strictness < $strictness ) {
			return;
		}

		// Simulate trigger_error
		if ( $strictness == self::LOGS_ERR ) {
			if ( ( $trace = debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ) ) && isset( $trace[ 0 ] ) ) {
				array_unshift( $args, sprintf( "File: %s, line: %s", $trace[ 0 ][ 'file'], $trace[ 0 ]['line'] ) );
			}
		}
	...
	public function ticket_button() {
		...
		if( curl_errno( $ch ) ) {
			...
			return $this->__( 'Failed to fetch the support form: Network Error' );
		}
...
HTML;
	 ...
	 * @param boolean Flag of manual update
	 * @return int
	 */
	public function updator( $updates, $manual = false ) {
		$current_version = $this->config( 'db_version', '0' );
		$last_verson = null;

		// Update has status in progress - last update failed
		if ( !$manual && $this->config( 'update_in_progress' ) ) {
			return self::UPDATE_MANUAL;
		}

		uksort( $updates, function( $a, $b ) { return version_compare( $a, $b ); } );

		$this->set_setting( 'update_in_progress', 1 );

		foreach( $updates as $version => $file ) {
			if ( version_compare( $version, $current_version, '>' ) ) {
				$this->log( sprintf( 'Updating to version %s', $version ) );

				if( $this->apply_update( $file ) ) {
					$last_verson = $version;
				}

				$this->log( sprintf( 'Successfully update to version %s', $version ) );
			}
		}

		$this->set_setting( 'update_in_progress', 0 );
		// $this->set_setting( 'db_vesrion', $last_verson );

		return self::UPDATE_SUCCESS;
	}

	/**
	 * Applies specific update
	 * @param string $file 
	 * @return boolean
	 */
	public function apply_update( $file ) {
		$status = self::UPDATE_FAILED;
	...
	function is_system_available() {
		static $system_available;

		if ( ! isset( $system_available ) ) {
