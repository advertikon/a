...
class Query {
	...
	public function run_query( $data ) {
		...
		if ( ! $data ) {
			ADK()->log( 'Failed to run query - dataset is empty', ADK()->log_error_flag );
			// throw new Exception( $mess );
			return $ret;
		}

		$query = ADK()->do_clean( function() use( $data ) { return ADK()->db->query( $data ); }, $error );

		if ( $error ) {
			ADK()->log( (string)$error, ADK()->log_error_flag );
		}
		...
		if ( $query ) {
			...
			// Select
			} elseif ( gettype( $query ) === 'object' && isset( $query->rows ) ) {
