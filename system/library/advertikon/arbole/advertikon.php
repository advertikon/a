...
class Advertikon extends \Advertikon\Advertikon {
	...
	public function get_product_id( $data ) {
		...
		if ( $q ) {
			if ( count( $q ) > 0 ) {
				...
				$this->update_customization( $product_id, $data );

			} else {
				$product_id = $this->clone_product( $data );
				$this->save_customization( $product_id, $data );
			}
		}
	...
	public function clone_product( $d ) {
		$product_id = $d['product_id'];
		$ret = false;

		$query = $this->db->query(
			"SELECT DISTINCT * FROM " . DB_PREFIX . "product p WHERE p.product_id = '" . (int)$product_id . "'" );
		...
		if ($query->num_rows) {
			...
			$ret = $this->addProduct($data);
		}

		return $ret;
	}

	public function update_product( $d ) {
		$product_id = $d['product_id'];
	...
	public function save_customization( $product_id, $data ) {
		$q = $this->q( [
			'table' => $this->customization,
			'query' => 'insert',
			'values' => [
